import type { Linter } from 'eslint'
import type { OptionsOverrides } from '../types'
import { GLOB_SRC } from '../constants'
import { loadPlugin } from '../plugins'

/**
 * JSDoc configuration options
 */
export type JSDocOptions = OptionsOverrides

/**
 * JSDoc configuration
 * Rules for JSDoc comments
 */
export async function jsdoc(options: JSDocOptions = {}): Promise<Linter.Config[]> {
	const { overrides = {} } = options

	const jsdocPlugin = await loadPlugin<typeof import('eslint-plugin-jsdoc')>('eslint-plugin-jsdoc')

	if (!jsdocPlugin) {
		return []
	}

	return [
		{
			name: 'eslint-sets/jsdoc',
			files: [GLOB_SRC],
			plugins: {
				jsdoc: jsdocPlugin as any,
			},
			rules: {
				'jsdoc/check-access': 'warn',
				'jsdoc/check-alignment': 'warn',
				'jsdoc/check-param-names': 'warn',
				'jsdoc/check-property-names': 'warn',
				'jsdoc/check-types': 'warn',
				'jsdoc/empty-tags': 'warn',
				'jsdoc/implements-on-classes': 'warn',
				'jsdoc/multiline-blocks': 'warn',
				'jsdoc/no-defaults': 'warn',
				'jsdoc/no-multi-asterisks': 'warn',
				'jsdoc/require-param-name': 'warn',
				'jsdoc/require-property': 'warn',
				'jsdoc/require-property-description': 'warn',
				'jsdoc/require-property-name': 'warn',
				'jsdoc/require-returns-check': 'warn',
				'jsdoc/require-returns-description': 'warn',
				'jsdoc/require-yields-check': 'warn',

				// User overrides
				...overrides,
			},
		},
	]
}
